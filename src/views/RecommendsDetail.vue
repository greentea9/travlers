   ...
   <v-container>
     ...
     <v-col cols="12">
        <v-card color="light-green lighten-5" >
          <v-card-title>
            <span class="fad">{{recommend.recommendSubject}}</span>
          </v-card-title>
          <v-card-subtitle>
            <div class="fas">
          ...
          <v-row>
            <v-col cols="12" md="2" class="pl-10 pr-0">
              <v-card-subtitle class="pt-0 pr-0">
                <span class="fad4">작성자: {{recommend.recommendUser}}</span>
              </v-card-subtitle>
            </v-col>
            <v-col cols="12" md="3"  class="pl-0">
              <v-card-subtitle class="pt-0 pl-0">
                <span class="fad4">날짜:{{recommend.recommendDate}}</span>
              </v-card-subtitle>
            </v-col>
          </v-row>
          <carousel :perPageCustom="[[1, 1],[768,2]]" :navigationEnabled="true" class="mx-7">
            <!--Slide v-for="(recommend,index) in recommends" :key="index" >-->
              <v-img class="white--text align-end mx-1" :src="recommend.recommendContentImg" height="500" >
              </v-img>
            <!--/Slide>-->
          </carousel>
          <div>
              ...
              <v-col cols="12" md="7">
                <v-card color="light-green lighten-4" class="ml-3">
                  <v-card-title class="mb-5">
                    <span class="fad3">추천지 이름</span>
                  </v-card-title>
                  <v-card-text >
                    <p style="line-height:1.5em" v-html="recommend.recommendContent"></p>
                  </v-card-text>
                </v-card>
              </v-col>
              <v-col cols="12" md="5">
                <v-img class="white--text align-end mr-3" :src="recommend.recommendContentImg" height="300" >
                </v-img>
              </v-col>
            </v-row>
          </div>
...
export default {
  ...
  created(){
    ...
    window.scroll(0, 0);
    //추천 상세 데이터 가져오기
    this.$http.get(`${this.dataURL}/detail/${this.$route.params.recommendStoreNo}`).then((result)=>{
      this.recommend = result.data;
      let date = this.recommend.recommendDate;
      let dateCut = date.toString().slice(0, 10);
      this.recommend.recommendDate = dateCut;
      //console.log(this.recommends[0])
    })
    //추천 댓글 데이터 가져오기
    this.$http.get(`${this.dataURL}/comment/${this.$route.params.recommendStoreNo}`).then((result)=>{
